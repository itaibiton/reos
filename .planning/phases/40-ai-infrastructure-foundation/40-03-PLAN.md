...
<task type="auto">
  ...
  <action>
The chat.ts action needs a query to get user by Clerk ID. Check if this exists in convex/users.ts, and add it if missing:

**Add to convex/users.ts** (if not already present):
...
This query allows the chat action to look up the user document from the Clerk identity.
  ...
  <verify>
```bash
grep -n "getByClerkId" /Users/Kohelet/Code/REOS/convex/users.ts
npx convex dev --once
```
Query should exist and Convex should compile.
  </verify>
