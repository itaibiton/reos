...
must_haves:
  ...
  artifacts:
    ...
    - path: "convex/serviceProviderProfiles.ts"
      provides: "submitForApproval, approveVendor, rejectVendor mutations; listPendingVendors, getMyApprovalStatus queries"
      exports: ["submitForApproval", "approveVendor", "rejectVendor", "listPendingVendors", "getMyApprovalStatus"]
...
<task type="auto">
  <name>Task 2: Approval workflow mutations/queries and profile photo mutations</name>
  <files>
    convex/serviceProviderProfiles.ts
    convex/users.ts
  </files>
...
6. **Modify `listByType` query**: After the existing `.withIndex("by_provider_type", ...)` and `.collect()`, add a filter:
   ```typescript
...
7. **Modify `getPublicProfile` query**: After getting the profile, before returning data, add a visibility check:
...
8. **Modify `getProvidersByType` query**: Same filter as listByType -- add `p.approvalStatus === "approved" || p.approvalStatus === undefined`.

9. **Modify `upsertProfile` mutation**: When creating a NEW profile (not updating), set `approvalStatus: "draft"` in the insert. When updating an existing profile, do NOT change approvalStatus (it stays as-is).

**users.ts -- Add profile photo mutations:**

10. **`generateProfilePhotoUploadUrl` mutation**:
    - No args
    - Auth check: verify authenticated
    - Return `await ctx.storage.generateUploadUrl()`
    - Follow exact same pattern as `dealFiles.generateUploadUrl`

11. **`saveProfilePhoto` mutation**:
...
13. **Modify `setUserRole` mutation**: Change the logic so that service providers (broker, mortgage_advisor, lawyer) now get `onboardingComplete: false` instead of `true`. This forces them through the vendor onboarding wizard before they can use the platform. Keep admin and investor logic unchanged.
...
5. `listByType` now filters by approvalStatus
6. `generateProfilePhotoUploadUrl` and `saveProfilePhoto` exist as mutations
7. `setUserRole` sets onboardingComplete: false for service providers
  </verify>
  <done>
All approval workflow mutations (submit, approve, reject) and queries (listPending, getMyApprovalStatus) are implemented. Existing directory queries (listByType, getProvidersByType, getPublicProfile) filter out unapproved vendors while grandfathering existing providers. Profile photo upload/save mutations are implemented. Service provider role selection now sets onboardingComplete: false to force onboarding wizard completion. Convex pushes without errors.
  </done>
</task>
...
- All 3 new queries compile: listPendingVendors, getMyApprovalStatus, getProfilePhotoUrl
