...
<task type="auto">
  <name>Task 2: Multi-step wizard, step components, route page, and i18n translations</name>
  <files>
    src/app/[locale]/(app)/onboarding/vendor-profile/page.tsx
    src/components/vendor/VendorOnboardingWizard.tsx
    src/components/vendor/steps/BasicInfoStep.tsx
...
- Map providerType from the user's already-set role (they chose role in the first onboarding step).
  ...
  <verify>
1. Navigate to `/onboarding/vendor-profile` (as a newly registered service provider) -- the 4-step wizard should render
...
6. Check that i18n works -- switch to Hebrew locale, all labels should be in Hebrew with RTL layout
  ...
  <done>
Multi-step vendor onboarding wizard at /onboarding/vendor-profile with 4 steps, all 12+ registration fields, profile photo upload, Zod validation per step, localStorage draft persistence, Convex draft saving on step advance, final submission calling submitForApproval, and full EN/HE translations. Service provider role selection redirects to the wizard. Completing the wizard submits for approval and redirects to dashboard.
  </done>
</task>
...
5. Submission creates a serviceProviderProfile with approvalStatus: "pending" and submittedAt timestamp
6. All text is translated in both EN and HE
7. Step transitions are animated with Framer Motion
