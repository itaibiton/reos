...
must_haves:
  truths:
    ...
    - "AI response appears token-by-token as it generates"
    - "Typing indicator visible while waiting for first token"
    - "Previous messages in conversation visible when returning to chat"
...
Verify the Message interface matches what useAIChat returns.
  ...
  <done>
ChatMessageList renders messages with smart scroll and typing indicator.
  </done>
</task>
...
- CHAT-04: Chat history displays previous messages (ChatMessageList with messages from hook)
</success_criteria>
