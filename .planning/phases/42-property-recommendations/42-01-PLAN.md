...
must_haves:
  truths:
    ...
    - "Search results only contain properties that exist in database"
...
<task type="auto">
  <name>Task 3: Register tool with agent and update instructions</name>
...
3. Update the agent instructions to include property recommendation guidance:
   ...
   - For each property, explain 2-3 reasons why it matches (budget fit, location preference, property type match)
...
Check agent.ts includes `searchProperties` in tools object.
  ...
  <done>
Agent has searchProperties tool registered, instructions include property recommendation guidance, never-hallucinate rule explicit.
  </done>
</task>
