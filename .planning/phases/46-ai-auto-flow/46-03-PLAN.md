...
must_haves:
  truths:
    ...
    - "Tool results render as interactive cards (not plain text)"
    - "Property cards have SaveAllButton below them"
    - "Provider cards are grouped by role with Add to Team buttons"
  artifacts:
    ...
    - path: "src/components/ai/ChatMessage.tsx"
      provides: "Tool result card rendering"
      contains: "PropertyCardRenderer"
    - path: "src/components/ai/PropertyCardRenderer.tsx"
      provides: "SaveAllButton integration"
      contains: "SaveAllButton"
    - path: "src/components/ai/ProviderCardRenderer.tsx"
      provides: "Role grouping with accordion"
  ...
  key_links:
    ...
    - from: "ChatMessage"
      to: "PropertyCardRenderer"
      via: "renders for assistant messages with toolCalls"
      pattern: "PropertyCardRenderer.*toolCalls"
    - from: "ChatMessage"
      to: "ProviderCardRenderer"
      via: "renders for assistant messages with toolCalls"
      pattern: "ProviderCardRenderer.*toolCalls"
    - from: "PropertyCardRenderer"
      to: "SaveAllButton"
      via: "renders when 2+ properties"
      pattern: "SaveAllButton.*propertyIds"
---

<objective>
Wire auto-greeting for mobile, verify questionnaire reminder persistence, verify Phase 42-43 card rendering components, and add translations.

Purpose: Ensure mobile has same auto-greeting behavior as desktop, questionnaire reminder persists until completion, and tool results render as interactive cards
Output: Mobile summary triggers auto-greeting, verified persistence behavior, verified card rendering, added i18n
</objective>
...
<task type="auto">
  <name>Task 2: Verify Phase 42-43 card rendering components</name>
  <files>
    src/components/ai/ChatMessage.tsx
    src/components/ai/PropertyCardRenderer.tsx
    src/components/ai/ProviderCardRenderer.tsx
  ...
  <action>
Verify the Phase 42-43 components are correctly wired for card rendering (AUTO-02, AUTO-03, AUTO-04):

**AUTO-02 Verification (cards not text):**
1. Read `src/components/ai/ChatMessage.tsx`
2. Confirm PropertyCardRenderer and ProviderCardRenderer are imported and rendered for assistant messages
3. Verify they receive `toolCalls` prop from the message
4. Pattern to find: `PropertyCardRenderer toolCalls={toolCalls}` and `ProviderCardRenderer toolCalls={toolCalls}`

**AUTO-03 Verification (SaveAllButton):**
1. Read `src/components/ai/PropertyCardRenderer.tsx`
2. Confirm SaveAllButton is imported
3. Confirm it renders when `properties.length >= 2`
4. Pattern to find: `SaveAllButton propertyIds={propertyIds}`

**AUTO-04 Verification (role grouping with Add to Team):**
1. Read `src/components/ai/ProviderCardRenderer.tsx`
2. Confirm Accordion component groups providers by role
3. Confirm roleLabels includes broker, mortgage_advisor, lawyer
4. Pattern to find: `AccordionItem key={role}` and `roleLabels`

**Verification commands:**
```bash
# AUTO-02: Cards render from tool results
grep -n "PropertyCardRenderer\|ProviderCardRenderer" src/components/ai/ChatMessage.tsx

# AUTO-03: SaveAllButton in PropertyCardRenderer
grep -n "SaveAllButton" src/components/ai/PropertyCardRenderer.tsx

# AUTO-04: Role grouping in ProviderCardRenderer
grep -n "AccordionItem\|roleLabels" src/components/ai/ProviderCardRenderer.tsx
```

**Expected output:** All patterns found, components correctly wired from Phase 42-43.

**If any pattern NOT found:** Document the gap and flag as blocker for human checkpoint.
  </action>
  <verify>
AUTO-02: `grep -c "PropertyCardRenderer\|ProviderCardRenderer" src/components/ai/ChatMessage.tsx` returns 4+ (imports + usages)
AUTO-03: `grep -c "SaveAllButton" src/components/ai/PropertyCardRenderer.tsx` returns 2+ (import + usage)
AUTO-04: `grep -c "AccordionItem" src/components/ai/ProviderCardRenderer.tsx` returns 1+
  </verify>
  <done>
Verified: ChatMessage renders PropertyCardRenderer and ProviderCardRenderer for assistant toolCalls
Verified: PropertyCardRenderer includes SaveAllButton for 2+ properties (AUTO-03)
Verified: ProviderCardRenderer groups by role with Accordion (AUTO-04)
All Phase 42-43 components correctly wired for auto-greeting card display
  </done>
</task>

<task type="auto">
  <name>Task 3: Verify questionnaire completion await chain</name>
...
<task type="auto">
  <name>Task 4: Verify IncompleteProfileReminder persistence behavior</name>
...
<task type="auto">
  <name>Task 5: Add translations for AI auto-greeting context</name>
  <files>
    messages/en.json
    messages/he.json
  </files>
...
3. Agent verified to support dual tool execution (Plan 01 Task 3)
4. AIChatPanel accepts autoGreet prop for auto-greeting trigger (Plan 02)
5. Desktop summary page passes autoGreet={true} to AIChatPanel (Plan 02)
6. Mobile summary passes autoGreet={true} to AIChatPanel (Plan 03)
7. Phase 42-43 card rendering verified: ChatMessage -> PropertyCardRenderer -> SaveAllButton, ChatMessage -> ProviderCardRenderer -> role accordion (Plan 03 Task 2)
8. Questionnaire reminder persists until completion (verified existing behavior)
  </what-built>
  <how-to-verify>
...
3. **Verify auto-greeting (CRITICAL - AUTO-01, AUTO-02):**
   - On summary page, AI should automatically send a greeting
   - **AUTO-02 CHECK:** AI should show property CARDS (not plain text bullets or markdown)
   - If you see text like "1. Property at 123 Main St..." instead of interactive cards, this is a FAILURE
   - Cards should have images, prices, and match reasons

4. **Verify property cards (AUTO-03):**
   - Property cards should appear with individual "Save" buttons on each card
   - **SaveAllButton CHECK:** Look for a "Save All" button BELOW the property cards (only appears for 2+ properties)
   - If no SaveAllButton visible with 2+ properties, this is a FAILURE

5. **Verify provider cards (AUTO-04):**
   - After properties, AI should show provider cards
   - **Role grouping CHECK:** Providers should be in expandable accordion sections labeled "Brokers", "Mortgage Advisors", "Lawyers"
   - **Add to Team CHECK:** Each provider card should have an "Add to Team" button
   - If providers show as flat list without role grouping, this is a FAILURE

6. **Verify quick replies (SUCCESS-07):**
   - Quick reply buttons should remain visible after auto-suggestions complete
   - Buttons like "Show me properties", "Build my team" should still be accessible

7. **Verify no re-trigger:**
   - Refresh the page
   - AI should NOT send another greeting (messages already exist)

8. **Verify persistence (skip test):**
...
**FAILURE CRITERIA (stop and report if any occur):**
- [ ] Properties show as text/markdown instead of interactive cards -> AUTO-02 FAIL
- [ ] No SaveAllButton visible with 2+ properties -> AUTO-03 FAIL
- [ ] Providers not grouped by role (flat list) -> AUTO-04 FAIL
- [ ] No "Add to Team" button on provider cards -> AUTO-04 FAIL
- [ ] AI only calls ONE tool (properties OR providers, not both) -> Dual execution FAIL
- [ ] Quick replies disappear after auto-greeting -> SUCCESS-07 FAIL

**SUCCESS CHECKLIST:**
- [ ] Property cards render with match reasons and individual Save buttons (AUTO-02)
...
- [ ] AI executes both searchProperties AND searchProviders tools (visible in response)
  </how-to-verify>
  <resume-signal>Type "approved" if all flows work correctly, or describe any issues found (especially if any FAILURE CRITERIA triggered)</resume-signal>
</task>
...
2. Phase 42-43 card rendering verified (ChatMessage -> renderers -> components)
3. Questionnaire await chain verified correct
4. IncompleteProfileReminder correctly persists (code review)
5. Translations valid JSON
6. End-to-end flow works (human verification)
7. AUTO-02: Tool results render as cards (not text)
8. AUTO-03: SaveAllButton renders below property cards
9. AUTO-04: Providers grouped by role with Add to Team buttons
10. SUCCESS-07: Quick reply buttons visible after auto-suggestions
</verification>

<success_criteria>
- Mobile summary triggers auto-greeting via autoGreet={true} prop
- Phase 42-43 components verified: ChatMessage renders card renderers, PropertyCardRenderer includes SaveAllButton, ProviderCardRenderer uses role accordion
...
- Tool results display as interactive cards, not plain text (AUTO-02)
...
- AI executes dual tool calls (searchProperties + searchProviders) in single response
