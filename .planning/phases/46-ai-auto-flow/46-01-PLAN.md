...
must_haves:
  truths:
    ...
    - "Empty message with allowEmpty=true does not create user bubble in chat UI"
    - "AI agent executes both searchProperties and searchProviders in single response"
  artifacts:
  ...
  key_links:
    ...
    - from: "system prompt"
      to: "dual tool execution"
      via: "explicit instruction"
      pattern: "BOTH tool calls"
---
...
3. For optimistic user message, skip adding it when allowEmpty is true and text is empty (we don't want to show an empty user message in the UI):
   ...
   - This ensures no empty user bubble appears in the chat UI
...
Verify in code: when allowEmpty=true and text is empty, optimisticUserMessage is NOT added to messages array (no empty user bubble in UI)
  </verify>
  <done>
useAIChat.sendMessage accepts optional `{ allowEmpty: true }` to send empty messages for auto-greeting
Empty messages skip optimistic UI update (no empty user bubble shown in chat)
Code explicitly guards: `if (text.trim())` before adding optimistic user message
  </done>
</task>
...
<task type="auto">
  <name>Task 3: Verify dual tool execution capability</name>
  <files>convex/ai/agent.ts</files>
  <action>
Verify the agent configuration supports multiple tool calls in a single response:

1. Read `convex/ai/agent.ts` and confirm:
   - Both `searchProperties` and `searchProviders` tools are registered
   - No configuration limits tool calls to 1 per response
   - Agent uses a model that supports parallel tool use (Claude 3.5 Sonnet or similar)

2. Grep for any `maxToolRoundtrips`, `maxSteps`, or `maxIterations` configuration that might limit tool usage:
   ```bash
   grep -n "maxTool\|maxStep\|maxIter" convex/ai/*.ts
   ```

3. If any limiting configuration exists that would prevent dual tool calls, document it.

4. Verify the system prompt pattern from Task 2 explicitly instructs dual execution:
   - The phrase "BOTH tool calls" should appear
   - The phrase "Do NOT wait for user prompts" should appear

**Expected outcome:** Agent is configured to allow multiple tool calls. If not, document what needs changing.

**Note:** This is a verification task. If the agent already supports multiple tools (which it should per Phase 42-43), no code changes needed - just confirm and document.
  </action>
  <verify>
Confirm both tools registered: `grep -c "searchProperties\|searchProviders" convex/ai/agent.ts`
Check for limiting config: `grep -n "maxTool\|maxStep\|maxIter" convex/ai/*.ts`
Verify system prompt includes "BOTH tool calls" instruction (from Task 2)
  </verify>
  <done>
Confirmed: Agent supports multiple tool calls in single response
Both searchProperties and searchProviders are registered tools
No configuration limits prevent dual tool execution
System prompt explicitly instructs "BOTH tool calls automatically"
  </done>
</task>
...
4. Empty message with allowEmpty=true does not create visible user bubble in chat UI
5. Agent is capable of executing multiple tool calls in single response
</verification>

<success_criteria>
- useAIChat.sendMessage("", { allowEmpty: true }) is valid and sends to server
- Empty message + allowEmpty=true skips optimistic user message (no empty bubble in UI)
...
- Agent configuration allows dual tool execution (searchProperties + searchProviders)
