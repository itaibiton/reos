...
// Group keys for translation
type NotificationGroup = "messages" | "deals" | "files" | "requests";

// Map notification types to groups
function getNotificationGroup(type: NotificationType): NotificationGroup {
  switch (type) {
    case "new_message":
      return "messages";
...
interface GroupedNotifications {
  messages: NotificationItemProps["notification"][];
  deals: NotificationItemProps["notification"][];
  files: NotificationItemProps["notification"][];
  requests: NotificationItemProps["notification"][];
}
...
export function NotificationsTab() {
  ...
  const groupedNotifications = useMemo<GroupedNotifications>(() => {
    ...
    for (const notification of notifications) {
      const group = getNotificationGroup(notification.type as NotificationType);
      groups[group].push(notification as NotificationItemProps["notification"]);
    }
  ...
  const groupOrder: NotificationGroup[] = ["messages", "deals", "files", "requests"];
